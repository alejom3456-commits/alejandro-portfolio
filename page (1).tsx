...
const TRIVELA_VIDEOS: { id: string; caption: string }[] = [
  ...
  },
  {
    ...
    caption: "“Salte maestro” — the fan-community thesis stretching past football, into basketball/sneaker culture",
  },
];
          ...
          <div className="flex flex-col gap-14">
            {c.sections.map((s) => (
              <div key={s.title} id={s.title.toLowerCase().replace(/\s+/g, "-")} className="scroll-mt-8">
                ...
                {c.slug === "trivela" && s.title === "Audience Learning" && (
                  <div className="mt-6 grid max-w-2xl grid-cols-2 gap-x-6 gap-y-5 border-t border-rule pt-6 sm:grid-cols-4">
                    {[
                      ["52", "Followers"],
                      ["513", "Total likes"],
                      ["7,992", "Top video views"],
                      ["10x+", "Interviews vs. solo pieces"],
                    ].map(([value, label]) => (
                      <div key={label} className="flex flex-col gap-1">
                        <span className="font-grotesk text-[22px] font-bold text-ink">{value}</span>
        ...
        <section className="bg-inkNavy px-6 py-16 md:px-10 md:py-20">
          <div className="mx-auto max-w-5xl">
            ...
            <div className="grid grid-cols-1 gap-6 sm:grid-cols-3">
              {TRIVELA_VIDEOS.map((video) => (
                <div key={video.id} className="overflow-hidden rounded-sm border border-cream/10 bg-black">
                  <blockquote
